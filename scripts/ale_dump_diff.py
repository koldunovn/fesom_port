#!/usr/bin/env python3
"""Diff ALE dump files between the C port and the Fortran reference.

Usage: ale_dump_diff.py <ref_dir> <tst_dir> [tag_substr]

Clone of scripts/kpp_dump_diff.py for the zstar-port dual instrumentation.
Both codes write, gated by FESOM_ALE_DUMP_DIR (steps 1..FESOM_ALE_DUMP_STEPS,
default 3):

    ale_dump_s<step>_<tag>_rank<R>.txt
    # step=.. tag=.. rank=R N=.. ncomp=C   (header)
    <gid> <v0> <v1> ... <v(C-1)>           (one line per owned entity, 1-based gid)

Node tags: forcing[water_flux,virtual_salt,relax_salt,real_salt_flux],
sshsolve[ssh_rhs,d_eta], hbar[hbar,hbar_old,ssh_rhs_old,eta_n],
Wvel (nl cols), hnode_new / hnode / Z_3d_n (nl-1 cols), zbar_3d_n (nl cols).
Element tags: pgf_x, pgf_y, helem (nl-1 cols), dhe (1 col).
...
marks real divergence (above FP-summation noise).
"""
import glob
import os
import re
...
# Pretty column names for the small fixed-layout tags. Full-column tags
# (Wvel, hnode_new, hnode, zbar_3d_n, Z_3d_n, helem, pgf_x, pgf_y) fall back
# to c0,c1,... where cK == layer (interface) K, 0-based.
# Keep in sync with the C fesom_ale_dump.c + the Fortran ale_dump_mod.
COLS = {
    "forcing":  ["water_flux", "virtual_salt", "relax_salt", "real_salt_flux"],
    "sshsolve": ["ssh_rhs", "d_eta"],
    "hbar":     ["hbar", "hbar_old", "ssh_rhs_old", "eta_n"],
    "dhe":      ["dhe"],
}

FNAME_RE = re.compile(r"ale_dump_s(\d+)_(.+)_rank(\d+)\.txt$")


def discover(directory):
    """Return {(step, tag): [files]} for a dump dir."""
    out = {}
    for f in glob.glob(os.path.join(directory, "ale_dump_s*_rank*.txt")):
